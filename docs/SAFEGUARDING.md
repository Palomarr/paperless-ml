...
**Team**: Training (Dongting), Serving (Yikai), Data (Elnath)
...
- **Quality gates during training.** Before any candidate model is registered in the MLflow Model Registry, `check_gate()` at [`paperless_training_integration/quality_gate.py`](https://github.com/gdtmax/paperless_training_integration/blob/main/quality_gate.py) reads the latest evaluation run's metrics and compares them against thresholds loaded from [`configs/quality_gate.yaml`](https://github.com/gdtmax/paperless_training_integration/blob/main/configs/quality_gate.yaml). Only on PASS does the gate call `mlflow.register_model()` at `quality_gate.py`, enforcing the rubric clause that saved models are registered only if they pass a quality gate.
...
- **Low-confidence flagging**: every HTR result carries an `htr_flagged` boolean when the sequence-level confidence falls below `HTR_CONFIDENCE_THRESHOLD` (default 0.5) in [`serving/src/fastapi_app/app.py:279`](../serving/src/fastapi_app/app.py). Flagged regions surface to the user for manual correction via the feedback UI at `/ml-ui/doc/<id>/`, preventing silent propagation of low-quality transcriptions to downstream search indexing.
...
- **Rollback controller executes + logs every trigger.** The webhook receiver at [`ops/rollback-ctrl/app.py`](../ops/rollback-ctrl/app.py) both (a) logs the alert name, status, severity, and labels when any alert fires AND (b) for alerts carrying the `rollback_trigger: "true"` label, executes the rollback by moving the `@production` alias on MLflow's `paperless-htr` registered model back one version and restarting ml-gateway via the Docker API. Every execution is logged with `ROLLBACK: paperless-htr @production moved vN -> vN-1 (trigger=<alertname>)`, which is the authoritative record of which model version was removed from production and why. Per-alertname 5-minute cooldown dedupes Alertmanager retries so the same alert firing twice does not cause double rollback.
- **Pipeline scheduler logs every promotion.** The scheduler at [`ops/pipeline-scheduler/scheduler.py`](../ops/pipeline-scheduler/scheduler.py) logs `PROMOTE: paperless-htr @production -> v<N>` every time a gate-passed model gets promoted to production. Combined with rollback-ctrl's demote lines, the scheduler + rollback-ctrl logs together are a complete audit of which version was serving at any point in time — symmetric forward/reverse paths both produce timestamped log lines.
